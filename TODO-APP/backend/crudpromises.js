...
// This is the hard core of our TODO app project. It also has most of the flaws/bugs of our backend application, since we eventually ran out of time to basically write it all again so that all the connection pool issues, transaction rollbacks and promise rejections would have been handled properly.
// Now there are some leaks, so that if one fetches information via curl commands or VS Code Rest Client, it doesn't always show error rejections. It works well with the frontend application, since we are catching many possible errors in both frontend and backend.
// I still wanted to include this in portfolio because I'm proud of us keeping this spaghetti together.
